...
const Api = () => {
    ...
    const fetchData = async () => {
        try {
            const response = await fetch(`http://api.zippopotam.us/${formData.state}/${formData.zipCode}`)
            if (response.ok) {
                const data = await response.json();
                console.log("API Response data: ", data);
                const placeNames = data.places.map(place => place['place name'].replace(/ x\)/g, ''))
                setFormData(prevFormData => ({
                    ...prevFormData, cities: placeNames, city: data.places[0].state
                }))
                setError('')
            } else {
    ...
    const handleZipCodeChange = (e) => {
        ...
        if ((formData.state === "cz" || formData.state === "sk") && value.length > 3) {
            value = value.slice(0, 3) + ' ' + value.slice(3);
        }

        setFormData(prevFormData => ({
            ...prevFormData, zipCode: value
        }))
    }

    const handleStateChange = (e) => {
        setFormData(prevFormData => ({
            ...prevFormData, state: e.target.value, zipCode: ''
        }))
    }
    ...
    return <div className='body-api'>
        ...
        <div className='main-container'>
            <form onSubmit={formSubmitHandler}>
                <input type='text' onChange={handleZipCodeChange} value={formData.zipCode} placeholder='např. 612 00'></input>
